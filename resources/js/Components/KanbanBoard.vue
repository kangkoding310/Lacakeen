...
watch(
    () => props.statuses,
    (statuses) => {
        columns.value = JSON.parse(JSON.stringify(statuses));
    },
...
const changed = (event: DragChangeEvent, column: ProjectStatusColumn) => {
    ...
    taskService.move(
        task.id,
        {
            ...
            ordered_ids: column.tasks.map((item) => item.id),
        },
        {
            preserveScroll: true,
            preserveState: true,
            only: ['statuses', 'flash', 'notifications'],
            onError: () => {
                task.status_id = oldStatus;
        ...
        >
            <header class="relative mb-3 flex min-h-9 items-center justify-between px-1">
                <div v-if="editingId === column.id" class="relative z-10 w-full pb-9">
                    ...
                    >
                        <button
                            class="grid h-7 w-8 place-items-center rounded-md text-emerald-600 hover:bg-emerald-50"
                            aria-label="Save column name"
                            @click="saveRename(column)"
                        >
                            <Check class="h-4 w-4" />
                        </button>
                        <button
                            class="grid h-7 w-8 place-items-center rounded-md text-slate-500 hover:bg-slate-100"
                ...
                <div v-else class="flex min-w-0 items-center gap-2">
                    ...
                    />
                    <button
                        class="truncate text-left text-xs font-bold text-slate-700 hover:text-blue-600"
                ...
                <details v-if="editingId !== column.id" class="relative">
                    <summary
                        class="list-none cursor-pointer rounded-lg p-1.5 text-slate-400 hover:bg-white"
                    >
