            ...
            <div class="grid gap-5 lg:grid-cols-[1fr_320px]">
                <main class="ui-card p-6 sm:p-8">
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <span
                            class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold capitalize"
                            >{{ task.priority }}</span
                        >
                    </div>
                    ...
                    <div class="ui-card p-5">
                        ...
                        <dl class="mt-5 space-y-4 text-sm">
                            <div class="flex justify-between gap-4">
                                ...
                                <dd class="font-semibold">{{ task.status.name }}</dd>
                            </div>
                            <div class="flex justify-between gap-4">
                                <dt class="text-slate-400">Due date</dt>
                                <dd class="font-semibold">{{ formatTaskDate(task.due_date) }}</dd>
                            </div>
                            <div>
                                <dt class="mb-2 text-slate-400">Assignees</dt>
                                <dd><AvatarStack :users="task.assignees" size="md" /></dd>
                            </div>
                            <div>
                                <dt class="mb-2 text-slate-400">Labels</dt>
                                <dd class="flex flex-wrap gap-1">
                                    <span
                                        v-for="label in task.labels"
                                        :key="label.id"
                                        class="rounded-lg px-2 py-1 text-xs font-semibold text-white"
                    ...
                    <div class="ui-card p-5">
                        ...
                        <div class="mt-4 space-y-2">
                            <a
                                ...
                                class="flex items-center justify-between rounded-lg bg-slate-50 p-3 text-xs font-semibold"
                                ><span class="truncate">{{ file.file_name }}</span
                                ><Download class="h-4 w-4"
                            /></a>
                            <p v-if="!task.attachments.length" class="text-xs text-slate-400">
                                No files uploaded.
                            </p>
                        </div>
                    </div>
