...
const boardStatuses = computed(() =>
    props.statuses.map((status) => ({
        ...
        tasks: status.tasks.filter((task) =>
            filteredTasks.value.some((match) => match.id === task.id)
        ),
    }))
);
const completed = computed(
    () => props.tasks.filter((task) => task.status?.name.toLowerCase() === 'completed').length
);
const completion = computed(() =>
    props.tasks.length ? Math.round((completed.value / props.tasks.length) * 100) : 0
);
...
const timelineStyle = (task) => {
    ...
    return {
        ...
        backgroundColor: task.status?.color || props.project.color,
    };
};
...
const selectTab = (id) => {
    ...
    window.history.replaceState(
        {},
        '',
        route('projects.show', { project: props.project.id, tab: id })
    );
};
const openCreate = (statusId = '') => {
    createDefaults.value = { project_id: props.project.id, status_id: statusId };
    createOpen.value = true;
};
const addStatus = () =>
    statusForm.post(route('statuses.store', props.project.id), {
        preserveScroll: true,
        onSuccess: () => {
            statusForm.reset();
...
const addMember = () =>
    member.post(route('projects.members.store', props.project.id), {
        preserveScroll: true,
    ...
    <AppLayout :title="project.name" section="Projects" :fullscreen="activeTab === 'kanban'">
        <div :class="activeTab === 'kanban' && 'flex h-full min-h-0 flex-col overflow-hidden'">
                ...
                <div class="mx-auto max-w-[1600px]">
                    <div class="flex h-11 items-center justify-between gap-4">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h1
                                    class="truncate text-base font-bold leading-tight text-slate-950"
                                >
                                    {{ project.name }}
                                </h1>
                                <p class="text-[10px] font-semibold text-slate-400">
                                    {{ project.prefix }} · {{ project.tasks_count }} tasks
                                </p>
                            </div>
                        </div>
                        <div ref="menuElement" class="relative">
                            ...
                            >
                                <button
                                    ...
                                    "
                                >
                                    <Pencil class="h-4 w-4" />Edit project</button
                                ><button
                                    v-if="canManage"
                                    class="flex w-full items-center gap-2 rounded-lg px-3 py-2 hover:bg-slate-50"
                                    @click="toggleArchive"
                    ...
                    <nav class="mt-1 flex gap-1 overflow-x-auto">
                        <button
                            v-for="item in tabs"
                            :key="item.id"
            ...
            <div
                :class="
                    activeTab === 'kanban'
                        ? 'flex min-h-0 flex-1 flex-col px-4 py-3 sm:px-6 lg:px-8'
                        : 'page-shell'
                "
            >
                <template v-if="activeTab === 'summary'">
                    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        >
                            ...
                            <p class="mt-2 text-3xl font-bold" :class="stat.color">
                                {{ stat.value }}
                            </p>
                        </div>
                    </div>
                                ...
                                <div v-for="status in statuses" :key="status.id">
                                    ...
                                    <div class="h-2 overflow-hidden rounded-full bg-slate-100">
                                        <div
                                            ...
                                            :style="{
                                                width: `${tasks.length ? (status.tasks.length / tasks.length) * 100 : 0}%`,
                        ...
                        <section class="ui-card overflow-hidden">
                            <div class="border-b border-slate-100 p-5">
                                ...
                                <p class="mt-1 text-xs text-slate-400">
                                    Manage access and project roles.
                                </p>
                            </div>
                            <form
                                v-if="canManage && availableMembers.length"
                            ...
                            >
                                ...
                                />
                                <AppSelect
                                    v-model="member.role_in_project"
                                    :options="roleOptions.slice(1)"
                                    :searchable="false"
                                    :can-clear="false"
                                /><button
                                    class="ui-button-primary px-3"
                            ...
                            <div class="max-h-80 divide-y divide-slate-100 overflow-y-auto">
                                ...
                                >
                                    ...
                                    <AppSelect
                                        ...
                                        :model-value="user.pivot.role_in_project"
                                        :options="roleOptions"
                                        :searchable="false"
                                        :can-clear="false"
                                        class="w-28"
                                    ...
                                    <span
                                        v-else
                                        class="text-[10px] font-bold capitalize text-slate-400"
                                        >{{ user.pivot.role_in_project }}</span
                                    ><button
                                        v-if="canManage && user.id !== project.created_by"
                ...
                <KanbanBoard
                    ...
                    @add-task="openCreate($event)"
                    @add-column="statusOpen = true"
                    ><template #empty>
                        <EmptyState
                            title="No workflow yet"
                            description="Add a status column to begin."
                        />
                    </template>
                </KanbanBoard>
                                ...
                                <td class="px-3 py-4">
                                    <span
                                        ...
                                        :style="{
                                            ...
                                            backgroundColor: `${task.status.color}12`,
                                        }"
                                        >{{ task.status.name }}</span
                                    >
                                </td>
                                <td class="px-3 py-4 text-xs font-bold capitalize">
                                    {{ task.priority }}
                                </td>
                                <td class="px-3 py-4">
                ...
                <div v-else class="ui-card overflow-x-auto">
                    <div class="min-w-[850px]">
                        ...
                        <div
                            v-for="task in filteredTasks"
                            :key="task.id"
                            class="grid grid-cols-[220px_1fr] border-b border-slate-100 text-xs last:border-0"
                        >
                            <button
                            ...
                            <div class="relative m-3 min-h-7 rounded-lg bg-slate-50">
                                <button
            ...
            <Modal
                :open="statusOpen"
                title="Add column"
                description="Create a new status for this project."
                @close="statusOpen = false"
            >
                <form class="space-y-4" @submit.prevent="addStatus">
                    <div>
                        <label class="ui-label">Status name</label
                        ><input
                    ...
                    <div>
                        ...
                        ><input v-model="statusForm.color" type="color" class="ui-input p-1" />
                    </div>
                    <p v-if="statusForm.errors.name" class="text-xs text-red-600">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        >
                            Cancel</button
                        ><button class="ui-button-primary" :disabled="statusForm.processing">
                            {{ statusForm.processing ? 'Adding…' : 'Add column' }}
                        </button>
                    </div>
                </form>
            </Modal>
                    ...
                    <div class="grid grid-cols-[1fr_80px] gap-3">
                        <div>
                            ...
                            ><input
                                ...
                                maxlength="12"
                                required
                                @input="edit.prefix = edit.prefix.replace(/[^a-zA-Z0-9]/g, '')"
                            />
                        </div>
                        <div>
                            <label class="ui-label">Color</label
                            ><input v-model="edit.color" type="color" class="ui-input p-1" />
                        </div>
                    </div>
                    <div>
                        <label class="ui-label">Description</label
                        ><textarea v-model="edit.description" class="ui-input min-h-24 py-2" />
                    </div>
                    <div class="flex justify-end gap-2">
