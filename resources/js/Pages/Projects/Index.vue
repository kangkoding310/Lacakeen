    ...
    <AppLayout title="Projects" section="Dashboard">
        <div class="page-shell">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Create, organize, archive, and manage project access.
                    </p>
                </div>
                <button v-if="canCreate" class="ui-button-primary" @click="createOpen = true">
            ...
            <div class="mt-6 inline-flex rounded-xl bg-slate-100 p-1">
                <button
                    v-for="status in ['active', 'archived']"
                    :key="status"
                    class="rounded-lg px-4 py-2 text-xs font-semibold capitalize"
                    :class="
                        filter === status ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500'
                    "
                    @click="filter = status"
                >
                    {{ status }}
            ...
            >
                <article
                    ...
                    :key="project.id"
                    class="ui-card group p-5 transition hover:-translate-y-0.5 hover:shadow-md"
                >
                    <div class="flex items-start justify-between">
                        ...
                        <div v-if="canManage(project)" class="flex gap-1">
                            <button
                            ...
                            >
                                <Archive
                                    ...
                                    class="h-3.5 w-3.5"
                                /><ArchiveRestore v-else class="h-3.5 w-3.5" /></button
                            ><button
                    ...
                    <Link :href="route('projects.show', project.id)" class="block"
                        ><h2
                            class="mt-5 flex items-center justify-between font-bold group-hover:text-blue-700"
                        >
                            {{ project.name }}<ArrowRight class="h-4 w-4 text-slate-300" />
                        </h2>
                        ...
                        >
                            <AvatarStack :users="project.members" /><span
            ...
            ><form class="space-y-4" @submit.prevent="submitCreate">
                ...
                <div class="grid grid-cols-[1fr_80px] gap-3">
                    <div>
                        ...
                        ><input
                            v-model="create.prefix"
                            class="ui-input uppercase"
                            maxlength="12"
                            placeholder="WEB"
                            required
                            @input="create.prefix = create.prefix.replace(/[^a-zA-Z0-9]/g, '')"
                        />
                    </div>
                    <div>
                        <label class="ui-label">Color</label
                        ><input v-model="create.color" type="color" class="ui-input p-1" />
                    </div>
                </div>
                <div>
                    <label class="ui-label">Description</label
                    ><textarea v-model="create.description" class="ui-input min-h-24 py-2" />
                </div>
