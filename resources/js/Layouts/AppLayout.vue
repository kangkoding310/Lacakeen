...
const toggleSidebar = () => {
    ...
    localStorage.setItem(
        "lacakeen-sidebar",
        collapsed.value ? "collapsed" : "open",
    );
};
...
const closeDropdowns = (event) => {
    if (!projectMenuElement.value?.contains(event.target))
        projectMenuOpen.value = false;
};
const showToast = () => {
...
<template>
    <div class="bg-slate-50" :class="fullscreen ? 'h-screen overflow-hidden' : 'min-h-screen'">
        <Transition enter-active-class="transition-opacity" enter-from-class="opacity-0"
            ...
            <button v-if="mobileOpen" cla ss="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm lg:hidden"
                aria-label="Close navigation" @click="mobileOpen = false" />
        </Transition>
        ...
        <aside
            ...
            <div class="flex h-[78px] items-center border-b border-slate-100 px-5"
                ...
                <Link :href="route('dashboard')" class="flex min-w-0 items-center gap-3">
                    ...
                    <div v-if="!collapsed" class="min-w-0">
                        <div class="flex items-center gap-2">
                            <span class="text-base font-bold tracking-tight text-slate-950">Lacakeen</span>
                        </div>
                ...
                <button v-if="!collapsed" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 lg:hidden"
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-5">
                <div v-for="(group, groupIndex) in [
                    { label: 'Dashboard', items: dashboardItems },
                    { label: 'Tools', items: toolItems },
                ]" :key="group.label" :class="groupIndex ? 'mt-7' : ''">
                    <p v-if="!collapsed"
                        class="mb-2 px-3 text-[10px] font-bold uppercase tracking-[.15em] text-slate-400">
                        {{ group.label }}
                    </p>
                    <div class="space-y-1">
                        <Link v-for="item in group.items" :key="item.label" :href="route(item.route)"
                            :title="collapsed ? item.label : undefined"
                            class="relative flex h-10 items-center rounded-xl text-sm font-medium transition" :class="[
                                collapsed
                                    ? 'justify-center px-0'
                                    : 'gap-3 px-3',
                                active(item.match)
                ...
                <div class="mt-7">
                    <div v-if="!collapsed" class="mb-2 flex items-center justify-between px-3">
                        ...
                        <div class="flex items-center gap-0.5">
                            <span class="group relative">
                                <button v-if="projectNavigation.canCreate"
                                    class="rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-blue-600"
                                    aria-label="Create project" @click="projectCreateOpen = true">
                                    <Plus class="h-4 w-4" />
                                </button>
                                <span
                                    class="pointer-events-none absolute bottom-full right-0 z-30 mb-2 whitespace-nowrap rounded-md bg-slate-950 px-2 py-1 text-[10px] font-medium normal-case tracking-normal text-white opacity-0 shadow-lg transition group-hover:opacity-100">Create
                                    project</span>
                            </span>
                            <span ref="projectMenuElement" class="group relative">
                                <button class="rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-blue-600"
                                    aria-label="Project options" @click="projectMenuOpen = !projectMenuOpen">
                                    <MoreHorizontal class="h-4 w-4" />
                                </button>
                                <span
                                    class="pointer-events-none absolute bottom-full right-0 z-30 mb-2 whitespace-nowrap rounded-md bg-slate-950 px-2 py-1 text-[10px] font-medium normal-case tracking-normal text-white opacity-0 shadow-lg transition group-hover:opacity-100">Project
                                    options</span>
                                <div v-if="projectMenuOpen"
                                    ...
                                    <Link :href="route('projects.index')"
                                        ...
                                        <FolderKanban class="h-4 w-4" />Manage
                                        Projects
                                    </Link>
                                </div>
                            </span>
                    ...
                    <p v-if="!collapsed && projectNavigation.recent.length"
                        class="mb-1 px-3 text-[10px] font-semibold text-slate-400">
                        Recent
                    </p>
                    <div class="space-y-1">
                        <Link v-for="project in projectNavigation.recent" :key="project.id"
                            :href="route('projects.show', project.id)" :title="collapsed ? project.name : undefined"
                            class="relative flex h-10 items-center rounded-xl text-sm font-medium transition" :class="[
                                collapsed ? 'justify-center' : 'gap-3 px-3',
                                active('projects.show') &&
                                    page.props.project?.id === project.id
                                    ? 'bg-blue-50 text-blue-700'
                                    : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950',
                            ]">
                            <span
                                class="grid h-6 w-6 shrink-0 place-items-center rounded-lg text-[9px] font-black text-white"
                                :style="{ backgroundColor: project.color }">{{ project.prefix.slice(0, 2) }}</span><span
                                v-if="!collapsed" class="truncate">{{
                                    project.name
                                }}</span>
                        </Link>
                        <Link :href="route('projects.index')" :title="collapsed ? 'Manage Projects' : undefined"
                            ...
                            <FolderKanban class="h-[18px] w-[18px]" /><span v-if="!collapsed">All projects</span>
                            <ChevronRight v-if="!collapsed" class="ml-auto h-4 w-4 text-slate-300" />
                        </Link>
                    </div>
                </div>
            </nav>
            ...
            <div class="border-t border-slate-100 p-3">
                ...
                <Link :href="route('settings')" class="flex h-10 items-center rounded-xl text-sm font-medium" :class="[
                    collapsed ? 'justify-center' : 'gap-3 px-3',
                    active('settings*')
                        ? 'bg-blue-50 text-blue-700'
                ...
                <div class="mt-3 flex items-center rounded-xl bg-slate-50 p-2"
                    :class="collapsed ? 'justify-center' : 'gap-3'">
                    <img :src="user.avatar ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}`
                        " :alt="user.name" class="h-9 w-9 shrink-0 rounded-xl object-cover" />
                    ...
                    <Link v-if="!collapsed" :href="route('logout')" method="post" as="button"
                        class="text-[10px] font-semibold text-slate-400 hover:text-red-500">Logout</Link>
                </div>
            </div>
            <button
                ...
                :aria-label="collapsed ? 'Expand sidebar' : 'Collapse sidebar'" @click="toggleSidebar">
                <ChevronRight v-if="collapsed" class="h-3.5 w-3.5" />
                <ChevronLeft v-else class="h-3.5 w-3.5" />
            </button>
        </aside>

        <div class="min-w-0 transition-[padding] duration-300" :class="[
            ...
            <header
                ...
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="hidden text-sm sm:flex">
                        <span class="text-slate-400">{{ section }}</span><span class="mx-2 text-slate-300">/</span><span
                            class="font-semibold text-slate-900">{{
                                title
                            }}</span>
                    </div>
                    <span class="truncate text-sm font-semibold sm:hidden">{{
                        title
                        }}</span>
                </div>
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <div class="relative">
                        <button class="ui-icon-button relative" @click="notificationOpen = !notificationOpen">
                            <Bell class="h-[18px] w-[18px]" /><span v-if="page.props.notifications?.unread"
                        ...
                        <div v-if="notificationOpen"
                            ...
                            <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                                <span class="text-sm font-bold">Notifications</span><span
                                    class="text-xs text-blue-600">{{
                                        page.props.notifications.unread
                                    }}
                                    unread</span>
                            </div>
                            <Link v-for="notification in page.props.notifications
                                .latest" :key="notification.id" :href="notification.data.url ||
                                    route('notifications')
                            ...
                            <Link :href="route('notifications')"
                                class="block px-4 py-3 text-center text-xs font-semibold text-blue-600">View all
                                notifications</Link>
                        </div>
                    </div>
